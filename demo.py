    ...
    def __init__(self, img_path):
        ...
        if 'query' in self.img_path:
            img_path_list = glob.glob(
                self.img_path + '/**/**/*.jpg', recursive=True)
            self.img_path_list = img_path_list
        elif 'db' in self.img_path:
            img_path_list = glob.glob(
                self.img_path + '/**/**/*.jpg', recursive=True)
            # sort images for db
            self.img_path_list = sorted(img_path_list, key=lambda x: int(
                os.path.splitext(os.path.basename(x))[0]))
        else:
            raise ValueError('img_path should be either query or db')
        assert len(
            self.img_path_list) > 0, f'No images found in {self.img_path}'
    ...
    def run(self, split: str = 'db') -> np.ndarray:
        ...
        with torch.no_grad():
            global_descriptors = np.zeros(
                (len(self.dataset), self.feature_dim))
...
def calculate_top_k(q_matrix: np.ndarray,
    ...
    similarity_matrix = np.matmul(q_matrix, db_matrix.T)

    # compute top-k matches
    # shape: (num_query_images, 10)
    top_k_matches = np.argsort(-similarity_matrix, axis=1)[:, :top_k]
...
def visualize(top_k_matches: np.ndarray,
    ...
    for q_idx, db_idx in enumerate(tqdm(top_k_matches, ncols=100, desc='Visualizing matches')):
        ...
        q_array = cv2.resize(q_array, img_resize_size,
        ...
        for i in db_idx.tolist():
            ...
            db_array = cv2.resize(db_array, img_resize_size,
                                  interpolation=cv2.INTER_CUBIC)
        ...
        cv2.imwrite(
            f'{visual_dir}/{os.path.basename(pred_q_path)}', result_array)
...
def main():
    ...
    db_global_descriptors = database_pipeline.run(
        split='db')  # shape: (num_db, feature_dim)
    query_global_descriptors = query_pipeline.run(
        split='query')  # shape: (num_query, feature_dim)
    ...
    record_matches(top_k_matches, query_dataset,
                   database_dataset, out_file='./LOGS/record.txt')

    # visualize top-k matches
    visualize(top_k_matches, query_dataset, database_dataset,
              visual_dir='./LOGS/visualize')
