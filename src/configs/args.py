...
def parse_args():
    ...
    parser.add_argument(
        "--backbone_arch",
        type=BackboneArch,
        choices=list(BackboneArch),
        default=BackboneArch.RESNET50,
    ...
    parser.add_argument(
        "--layers_to_crop", type=list, default=[], help="Layers to crop in the backbone"
    )
    parser.add_argument(
        "--agg_arch", type=str, default="ConvAP", help="Aggregator architecture"
    )
    parser.add_argument(
        "--agg_config", type=dict, default={}, help="Configuration for the aggregator"
    )
    ...
    parser.add_argument(
        "--warmup_steps", type=int, default=600, help="Number of warmup steps"
    )
    parser.add_argument(
        "--milestones",
        type=list,
    ...
    parser.add_argument(
        "--loss_name",
        type=LossName,
        choices=list(LossName),
        default=LossName.MULTI_SIMILARITY_LOSS,
    ...
    parser.add_argument(
        "--image_size", type=tuple, default=(320, 320), help="Image size"
    )
    parser.add_argument(
